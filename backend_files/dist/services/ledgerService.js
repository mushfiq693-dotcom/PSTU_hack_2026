...
class LedgerService {
    ...
    `;
        ...
        const countRes = await db_1.pool.query(countQuery, walletId ? [walletId] : []);
        const total = parseInt(countRes.rows[0].count, 10);
        const dataRes = await db_1.pool.query(dataQuery, [...params, limit, offset]);
        const entries = dataRes.rows;
     ...
     */
    static async verifySystemAudit() {
        const debitRes = await db_1.pool.query(`
      SELECT COALESCE(SUM(amount), 0) as total_debit, COUNT(*) as count 
      FROM ledger_entries 
      WHERE entry_type = 'DEBIT'
    `);
        const creditRes = await db_1.pool.query(`
    ...
    `);
        ...
        const totalWalletsRes = await db_1.pool.query(`
      SELECT COALESCE(SUM(balance), 0) as total_balance 
      FROM wallets 
      WHERE user_id != 'usr_system_treasury'
    `);
        const totalTransactionsRes = await db_1.pool.query('SELECT COUNT(*) as count FROM transactions');
