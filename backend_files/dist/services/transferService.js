...
class TransferService {
     ...
     */
    static async executeTransfer(params) {
        const { senderId, receiverId: initialReceiverId, receiverPhone, amountPoisha, note = 'P2P Transfer', category = 'General', type = 'TRANSFER', idempotencyKey } = params;
        // 1. Validation Checks
        if (!amountPoisha || amountPoisha <= 0 || !Number.isInteger(amountPoisha)) {
            const error = new Error('Transfer amount must be a positive integer in Poisha.');
        ...
        if (!resolvedReceiverId && receiverPhone) {
            const receiverUser = await db_1.pool.query('SELECT id FROM users WHERE phone = $1', [receiverPhone]);
            if (receiverUser.rows.length === 0) {
                const error = new Error(`Recipient with phone number '${receiverPhone}' was not found.`);
        ...
        if (!resolvedReceiverId) {
            const error = new Error('Recipient user ID or phone number is required.');
            error.statusCode = 400;
            error.errorCode = 'RECIPIENT_REQUIRED';
            throw error;
        }
        if (senderId === resolvedReceiverId) {
            const error = new Error('Self-transfers are not permitted. Please choose a different recipient.');
            error.statusCode = 400;
        ...
        try {
            ...
            const walletsRes = await client.query(`SELECT id, user_id, balance, status 
         FROM wallets 
         WHERE user_id = ANY($1::text[]) 
         ORDER BY user_id ASC 
         FOR UPDATE`, [userIdsToLock]);
            ...
            if (senderWallet.status !== 'ACTIVE') {
                const error = new Error(`Sender wallet is currently ${senderWallet.status}.`);
            ...
            if (receiverWallet.status !== 'ACTIVE') {
                const error = new Error(`Receiver wallet is currently ${receiverWallet.status}.`);
                error.statusCode = 403;
            ...
            if (currentSenderBalance < amountPoisha) {
                ...
                const error = new Error(`Insufficient funds. Available balance: ৳${currentBdt}, Required: ৳${reqBdt}.`);
                error.statusCode = 400;
         ...
         WHERE id = $2`, [senderNewBalance, senderWallet.id]);
            await client.query(`UPDATE wallets 
         SET balance = $1, updated_at = CURRENT_TIMESTAMP 
         WHERE id = $2`, [receiverNewBalance, receiverWallet.id]);
        ...
        VALUES ($1, $2, $3, $4, $5, $6, 0, $7, $8, 'SUCCESS', $9, CURRENT_TIMESTAMP)`, [
            ...
            const debitLedgerId = (0, uuid_1.v4)();
            const creditLedgerId = (0, uuid_1.v4)();
            // DEBIT Sender
            await client.query(`INSERT INTO ledger_entries (id, transaction_id, wallet_id, entry_type, amount, balance_after, created_at)
         VALUES ($1, $2, $3, 'DEBIT', $4, $5, CURRENT_TIMESTAMP)`, [debitLedgerId, txId, senderWallet.id, amountPoisha, senderNewBalance]);
            // CREDIT Receiver
            await client.query(`INSERT INTO ledger_entries (id, transaction_id, wallet_id, entry_type, amount, balance_after, created_at)
         VALUES ($1, $2, $3, 'CREDIT', $4, $5, CURRENT_TIMESTAMP)`, [creditLedgerId, txId, receiverWallet.id, amountPoisha, receiverNewBalance]);
            ...
            if (idempotencyKey) {
                ...
                await client.query(`INSERT INTO idempotency_records (key, user_id, status_code, response_body, created_at)
           VALUES ($1, $2, 200, $3, CURRENT_TIMESTAMP)
           ON CONFLICT (key) DO UPDATE SET response_body = EXCLUDED.response_body`, [idempotencyKey, senderId, JSON.stringify(cachedPayload)]);
            }
