...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  const { currentUser, allUsers, switchUser, resetDemoData, isLoading, isAuthenticated, logout } = useAuth();
  ...
  const navItems = [
    ...
    { id: 'send' as TabType, label: 'Send', icon: ArrowRightLeft },
    { id: 'requests' as TabType, label: 'Requests', icon: UserCheck },
    { id: 'splits' as TabType, label: 'Split Bills', icon: Receipt },
    { id: 'connections' as TabType, label: 'Friends & Family', icon: Users2 },
    { id: 'ledger' as TabType, label: 'Ledger', icon: FileSpreadsheet },
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-slate-800/80 bg-[#090d16]/90 backdrop-blur-2xl transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-2">
        ...
        <nav className="hidden xl:flex items-center bg-slate-950/80 p-1.5 rounded-2xl border border-slate-800 shadow-inner overflow-x-auto">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors flex items-center gap-1.5 z-10 shrink-0 ${
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <div className="text-right hidden sm:block">
                <div className="text-xs font-bold text-slate-200 group-hover:text-emerald-400 transition-colors truncate max-w-[110px]">
                  ...
                  <div className="mt-2 pt-2 border-t border-slate-800 space-y-1">
                    ...
                    {logout && (
                      <button
                        ...
                        className="w-full py-1.5 px-2 rounded-lg text-[11px] text-rose-400 hover:bg-rose-500/10 transition-colors flex items-center justify-center gap-1.5 font-medium"
                      >
      ...
      {/* Responsive Secondary Tab Bar for medium / mobile screens */}
      <div className="flex xl:hidden border-t border-slate-800 bg-slate-950/95 px-2 py-1.5 justify-around overflow-x-auto">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`flex flex-col items-center gap-1 p-2 rounded-xl text-[10px] font-semibold transition-all shrink-0 ${
